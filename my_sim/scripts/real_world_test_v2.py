...
class chargingDock:
    ...
    def linear_vel(self):
        ...
        # try:
        #     return 0.08*(sqrt(self.avgRange1**2 - 0.06**2))
        # except:
        #     return 0.08*(sqrt(self.avgRange2**2 - 0.06**2))
    ...
    def test_vel(self):
        if self.angle_mid < 0:
            return (3.14159 + abs(self.angle_mid))
        return (3.14159 - abs(self.angle_mid))
    ...
    def move_to_dock(self, radius):
        ...
        while self.range_mid > radius:
            ...
            #     rVel = -0.5*self.angular_vel()
            #     lVel = 0.5*self.angular_vel()
            ...
            while self.range_mid > radius and abs(self.avgRange1 - self.avgRange2) >= distance_tolerance:
                ...
                self.rwheel_publisher.publish(rVel)
                self.lwheel_publisher.publish(lVel)
            # if abs(self.angle_mid - 3.14159) > distance_tolerance2:

            #     if self.range_mid > radius and self.avgRange1 > self.avgRange2:
            #         rVel = self.linear_vel() + 1.5*abs(self.angular_vel()/self.constant)
            #         lVel = self.linear_vel() - 1.5*abs(self.angular_vel()/self.constant)
    ...
    def callBack(self, data):
        ...
        for index, value in enumerate(lidarArray):
            if self.flag3 == False and value > 750 and value < 1200: # Intensity range
                #flag = False
            ...
            if self.flag3 == True and value > np.amax(lidarArray) - 300 and value < np.amax(lidarArray) and (index < 200 or index > 1600): # Intensity range
                indexArray.append(index)
